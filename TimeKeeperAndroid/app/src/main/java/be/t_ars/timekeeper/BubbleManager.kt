...
class BubbleManager(private val context: Context) {
    ...
    fun showBubble() {
        ...
        val isPlaylistInBubble = getBoolPreference(context, kPLAYLIST_IN_BUBBLE, false)
        val bubbleIntent = if (isPlaylistInBubble)
            Intent(context, PlaylistBubbleActivity::class.java)
        else
            Intent(context, BubbleActivity::class.java)
        val bubblePendingIntent =
            PendingIntent.getActivity(context, 0, bubbleIntent, PendingIntent.FLAG_UPDATE_CURRENT)
        ...
        val bubbleData = Notification.BubbleMetadata
            ...
            .setDesiredHeight(if (isPlaylistInBubble) 500 else 180)
